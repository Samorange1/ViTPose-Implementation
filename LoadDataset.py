...
class LoadDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        center = self.transform_to_tensor(image_data["center"].reshape(1,2))
        scale = self.transform_to_tensor(image_data["scale"].reshape(1,2))
        target = self.transform_to_tensor(target)
        target_weight = self.transform_to_tensor(target_weight)

        return image, target, target_weight, center, scale, img_path 
